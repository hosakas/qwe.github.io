...
//Äîáàâëåíèå ýëåìåíòîâ è óäàëåíèå ýëåìåíòîâ.
...
console.error('$Create: Ýëåìåíò '+ID+' óæå ñóùåñòâóåò');
return null;
}
}
...
if($id(ToID) != null){
 ...
 if (result != null){
  ...
  console.warn('$Add: Ýëåìåíò '+ID+' íå ñîçäàí.');
 }
 return result;
} else {
 console.error('$Add: Ýëåìåíò '+ToID+' íå íàéäåí.');
 }
}
...
function API_IMGURUPLOAD(file) {
	...
	xhr.onerror = function(){
		alert('Âî âðåìÿçàãðóçêè ôàéëà ïðîèçîøëà îøèáêà: ' + xhr.status);
	};
...
function API_CREATEIMGURUPLOADWELL() {
	...
	window.ondrop = function(e) {
		...
		if(e.dataTransfer.files[0].type.match(/image.*/)){
			...
			alert('Ìîæíî çàãðóæàòü òîëüêî èçîáðàæåíèÿ.');
			this.value = '';
		}
	}
	IMGUR_FILEINPUTBTN = $Add('button', 'imgur_fileinputbtn', 'btn btn-sm btn-default', 'imgur_upload_well');
	IMGUR_FILEINPUTBTN.innerHTML = 'Çàãðóçèòü èçîáðàæåíèå';
...
// Îáðàáîòêà ñîîáùåíèé ïåðåä îïðàâêîé
...
// èíèöèàëèçàöèÿ ñìîòðåëêè êàðòèíîê.
API_IMAGEVIEWERINIT();
// Îáðàáîòêà êëèêà ïî êëàññó îáúåêòîâ.
...
// îáðàáîòêà ñîîáùåíèé â ñòðîêå ââîäà âî âðåìÿ ïå÷àòè.
...
API_CHATLOADFIX();
 ...
 $(".navbar-header").html("<img src=https://i.gyazo.com/83ad5721b6ec5d9d7631e234a2c28607.png style='position: relative; top: 7px;'>");
